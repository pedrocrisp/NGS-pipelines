...
closestBed -D "ref" -a -iu $outFolder/$sample.plus_sorted.bed -b $reference > $outFolder/$sample.plus.dist.bed
closestBed -D "ref" -a -iu $outFolder/$sample.minus_sorted.bed -b $reference > $outFolder/$sample.minus.dist.bed
#remove intermediate file
rm -rv $outFolder/*_sorted.bed

#subset to 500bp
awk -F$"\\t" '$NF<1 && $NF>-500' $outFolder/$sample.plus.dist.bed > $outFolder/$sample.plus.dist.1k.bed
awk -F$"\\t" '$NF>-1 && $NF<500' $outFolder/$sample.minus.dist.bed > $outFolder/$sample.minus.dist.1k.bed
...
then
# I'm not sure the nonstranded options will work the way it should...
...
#subset to 500bp
awk -F$"\\t" '$NF<1 && $NF>-500' $outFolder/$sample.dist.bed > $outFolder/$sample.dist.1k.bed
