...
usage="USAGE:
03-runner.sh <number of threads> <min length> <max length> <error rate>"
...
echo "initiating $1 parallel cutadapt adapter removal jobs, min/max length after filtering $min, $max, error rate $error_rate"
fi
########## Run #################
...
findSamples | parallel -j $threads bash $script {} $min $max $error_rate \>logs/${outdir}.${timestamp}/{}.log 2\>\&1
